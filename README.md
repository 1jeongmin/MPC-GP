...
| **물리 게이트 테스트 68개** | "코드가 도는지"가 아니라 "물리가 맞는지"를 본다 |
...
시나리오: `ay4` `ay6` `dlc` (합성) / `rt_ay4` `rt_ay6` (트랙) / `rtn_ay4` `rtn_ay6` (트랙+잡음)

**레이싱 트랙 데이터**는 `data/path/path_data_racetrack.mat`에 두어야 한다
(강의 실습 배포본. `data/`는 git 제외이므로 별도 복사 필요).
config에 sha256을 고정해 두어 파일이 바뀌면 즉시 중단된다.
