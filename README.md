...
현재 저장소에는 브랜치가 둘이고, **아직 병합하지 않았다.** 케이스 간 순위가 바뀌는
변경이라, 어느 쪽을 주 결과로 삼을지 정하기 전에 합치면 두 구성을 나란히 비교할 수
없게 되기 때문이다.

| 브랜치 | 제어 피드백으로 받는 상태 | 시나리오 접두사 |
|---|---|---|
| `main` | **생측정값** $y_k$ 를 그대로 (아래 §4 수치는 전부 이 기준) | `rtn_*` |
| `feature/state-estimator-feedback` | 명목 4상태 KF로 **한 번 필터링한** $\hat x_k$ | `rtf_*` |

**차이의 본질은 필터의 역할 분리다.** `main`에서는 KF 케이스조차 필터 출력 $\hat x$를
버리고 외란 추정 $\hat d$만 모델 보정에 썼다 — 필터가 **모델 보정** 역할만 하고
**상태추정** 역할은 하지 않았다. 브랜치는 세 케이스 **모두**에 동일한 상태추정기를
붙여 그 역할을 되돌린다(세 케이스가 같은 필터를 쓰므로 통제는 그대로 유지된다).

브랜치가 추가하는 것 — 기존 모듈 수정 없이 **주입**으로만 붙는다:

| 추가 | 내용 |
|---|---|
| `src/estimation/state_estimator.py` | 명목 4상태 KF. `AugmentedKF`의 필터 코어를 **상속 재사용**(복제 아님)하고, `d_hat`·`P_d_block` 접근은 명시적으로 막아 역할 혼동을 차단 |
| `configs/state_kf/default.yaml` | $Q$를 **상태추정 RMSE** 기준으로 7점 그리드 튜닝(U자 곡선, 0.1이 최적). 추종 RMS로 튜닝하지 않은 것은 §8의 「튜닝 목적함수 불일치」를 반복하지 않기 위해서다 |
| `configs/experiment/part1_*_rtf_*.yaml` 9개 | 3케이스 × `rtf_ay4`/`rtf_ay6`/`rtf_rturn` |
| `runner.py` / `assemble.py` | `state_estimator` 주입 인자 + `x_fb`·`skf_P_diag` 로깅, 배선 |

> **왜 EKF가 아니라 KF인가**: 필터 코어는 `ca.jacobian`으로 야코비안을 자동 생성하는
> EKF 구조지만, 명목 선형 모델 + 전상태 측정이라 야코비안이 상수로 나온다(두 작동점의
> $F$ 차이가 정확히 0). 선형계 위의 EKF는 KF와 같으므로 정직히 KF라 부른다.
> **비선형 플랜트 모델을 필터에 주지 않는다** — 추정기만 진짜 모델을 알게 되면
> "모델 오차를 어떻게 다루나"라는 실험 질문 자체가 사라진다.

측정된 차이 세 가지:

- **조향 포화가 사라진다.** 잡음 시나리오의 고질적 문제였던 "조향 변화율이 전체 시간의
  ~47%를 액추에이터 속도한계에 붙어 있음"(§9)이 **0%**가 된다.
- **절대 성능은 전부 개선되지만 순위가 뒤집힌다.** 강도외삽에서 KF −46.8% vs GP −9.1%로
  GP가 크게 진다. 다만 형상외삽에서는 필터링 후에도 GP가 이긴다 — 뒤집히는 건 강도축뿐이다.
- **`mpc_only`는 오히려 나빠진다**(+17~35%). 필터는 오차를 줄이는 대신 백색잡음을
  **상관된 지연**으로 바꾸는데(lag-1 자기상관 0.99), 백색잡음은 MPC 지평에서 상쇄되지만
  지연은 상쇄되지 않기 때문이다. 모델 보정이 있는 KF·GP는 그 지연의 원인(같은 모델 오차)을
  보정해 이득을 본다.

→ 그래서 남은 결정은 **"어느 피드백 구성을 주 결과로 삼는가"** 이고, §8에 올려 두었다.
이것이 정해지기 전에는 병합하지 않는다.

---

## 4. 주요 결과

아래 수치는 전부 `main` 기준, 즉 **생측정 피드백** 구성이다(위 브랜치 표 참조).
...
0. **★ 어느 피드백 구성을 주 결과로 삼을 것인가** (최우선 — 이게 정해져야
   `feature/state-estimator-feedback`을 병합한다). 생측정(`main`, `rtn_*`) vs
   필터링(`rtf_*`) — §3의 브랜치 표 참조. 세 선택지: **(a)** 필터링을 주 구성으로
   승격하고 생측정을 "필터가 없으면 이렇게 된다" ablation으로 둔다(현실 차량 구성에
   가장 가깝지만 GP 우위 서사가 약해진다) / **(b)** 생측정 유지, 필터링은 부록(기존
   결론이 보존되나 "왜 측정을 필터링하지 않았나"에 답해야 하고 47% 조향 포화가 근거로
   불리하다) / **(c)** 둘 다 병기하고 **"상태추정 품질이 GP·KF의 우열을 뒤집는다"**
   자체를 발견으로 서술한다(정직하고 데이터가 이미 있으나 논지가 복잡해진다).
   판단 보조: 형상축에서는 필터링 후에도 GP가 이기고 강도축에서만 뒤집힌다.
...
- **잡음 시나리오에서 조향 변화율이 전체 시간의 ~47%를 액추에이터 속도한계에서 보낸다**
  ...
  `feature/state-estimator-feedback` 브랜치에서 이 포화는 0%로 해소되지만, 그 대신
  케이스 간 순위가 바뀐다(§3의 브랜치 표).
