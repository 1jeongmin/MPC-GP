...
## 여기서 멈춘다 — Phase 6 이전에 결정할 것

**Phase 6 이후 프롬프트는 지금 쓸 수 없다.** GP 커널·하이퍼파라미터 사전·dictionary
크기 후보를 정하려면 Phase 5에서 측정한 **잔차의 크기와 공간 분포를 먼저 봐야** 하기
때문이다. 숫자를 보기 전에 프롬프트를 쓰면 추측으로 스펙을 채우게 된다.

결정 항목의 **단일 소스는 `handoff.md`의 「미결 결정」 표**다. 여기에 복제하지 않는다.
Phase 5 결과를 들고 와서 그 표를 채운 뒤 Phase 6 프롬프트를 작성한다.
...
### Phase 6~10 로드맵 (프롬프트는 나중에)

- **Phase 6** — EKF 비교군. `ekf-baseline.md`의 공정성 요구를 먼저 읽고 설계.
- **Phase 7** — offline GP. 학습 → CasADi 변환 → `mpc_gp` 결합.
  게이트: GPyTorch 예측과 CasADi 변환본 일치, 학습 데이터 밖에서 사후 std 증가.
- **Phase 8** — **Part 1 실험 실행**. MPC only / +EKF / +GP.
  주 산출물은 추종오차가 아니라 **GP 사후 std vs EKF `P` 대각의 공간 지도**와
  예측구간 커버리지다.
